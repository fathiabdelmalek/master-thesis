...
for folder_path in [characters_folder_path, words_folder_path, dynamic_folder_path, static_folder_path]:
    for sub_folder_path in folder_path.iterdir():
        if sub_folder_path.is_dir():
            for file_path in sub_folder_path.glob('*.csv'):
                if folder_path == words_folder_path:
                    if file_path.name not in words_files_set:
                        try:
                            file_path.unlink()
                        except OSError as e:
                            print(e)
                elif folder_path == dynamic_folder_path:
                    if file_path.name not in dynamic_files_set:
                        try:
                            file_path.unlink()
                        except OSError as e:
                            print(e)
                elif folder_path == characters_folder_path:
                    if file_path.name in words_files_set:
                        try:
                            file_path.unlink()
                        except OSError as e:
                            print(e)
                elif folder_path == static_folder_path:
